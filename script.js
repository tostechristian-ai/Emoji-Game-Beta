    ...
    merchants.push({
        ...
        function createPlayer2Weapon() {
            ...
            fireWeaponFromPool({
                ...
                size: 38, speed: 5.04,
                angle: player2.gunAngle,
                dx: Math.cos(player2.gunAngle) * 5.04, dy: Math.sin(player2.gunAngle) * 5.04,
                lifetime: Date.now() + 2000, hitsLeft: 1, hitEnemies: []
            });
            playSound('playerShoot');
        }
    ...
    if (distance < (player.size / 2) + (currentMerchant.size / 2)) {
        ...
        break;                   // Stop checking for this frame
            ...
            if (player2 && player2.active) {
                ...
                let p2isShooting = false;
                // Numpad 8 is up
                if (keys['8']) { player2.gunAngle = -Math.PI / 2; p2isShooting = true; }
                // Numpad 5 is down
                if (keys['5']) { player2.gunAngle = Math.PI / 2; p2isShooting = true; }
                // Numpad 4 is left
                if (keys['4']) { player2.gunAngle = Math.PI; p2isShooting = true; }
                // Numpad 6 is right
                if (keys['6']) { player2.gunAngle = 0; p2isShooting = true; }
                
                // Fire the weapon with the correct angle
                if (p2isShooting && now - player2.lastFireTime > player2.fireInterval) {
    const angle = player2.gunAngle;
    createWeapon(player2, angle); // This is the correct line.
...
// --- YOUR NEW CODE FOR PLAYER 2 GOES DIRECTLY BELOW THIS LINE ---

// Firing logic for Player 2
if (player2 && player2.active) {
    // Determine the aiming direction based on numpad keys
    if (keys['8']) p2aimDy -= 1;
    if (keys['2']) p2aimDy += 1;
    if (keys['4']) p2aimDx -= 1;
    if (keys['6']) p2aimDx += 1;

    // Check if any aiming key is pressed
    let p2isShooting = p2aimDx !== 0 || p2aimDy !== 0;
